...
## 実際に記事を 1 本作った（2026-09-23）

**Gemini に投げて Qiita 記事を 1 本生成し、論調の無い記事が読み物として成立するかを確かめた。**
本番の候補ストア・記事ディレクトリ・Notion には触れていない（すべて一時パスに向けた）。

結果: **成立する。** 見出しは `元記事の要旨` → `imo` → `用語` → `出典` の順で、
論調の節が無くても流れが不自然にならない。要旨 4 件は元記事の主張を正確に追えており、
用語も的確だった（法務用語の「依拠性」が入り、分野を問わない方針が効いている）。

**そこで見つかった表示の問題（直した）**

| 問題 | 直したこと |
|---|---|
| 出典欄に「Qiita の議論」が出るが、**元記事とまったく同じ URL**を指す。しかもコメント 0 件なので、議論があるかのように読める | `site/src/lib/sources.ts` に `hasSeparateDiscussion()` を足し、`discussionUrl === sourceUrl` なら出さないようにした。**ソース名で分岐しない**（値を見れば判断できるので、ソースを足してもここは直さなくてよい） |

確認した表示:
```
見出し: ['元記事の要旨', 'imo', '用語', '出典']
出典欄:
  - 元記事: 生成AI を使ったら著作権侵害になる？ — …
  - はてなブックマークのコメント        ← 「Qiita の議論」が消えている
メタ行: Qiita 9 LGTM / 0 コメント        ← scoreUnit が LGTM になっている
```
