...
- [ ] 出力された `title` が 40〜60 文字で、`【】` や感情語を含んでいないことを目視で確認した  ← **未達**: GEMINI_API_KEY が無く実呼び出しができないため未検証
- [ ] 出力された `discourse` に、反応の原文がそのまま含まれていないことを目視で確認した  ← **未達**: GEMINI_API_KEY が無く実呼び出しができないため未検証
