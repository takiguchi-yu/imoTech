...
| **公式 OTF（このファイル）** | **欠けなし**。速度も同等（スクリプト単体で測って 1 枚目 338 ms、2 枚目以降 約 330 ms/枚。大半は描画側。ビルドの中では 1 枚目が約 0.35〜0.5 秒（単独ビルドで実測。別のビルドが並行していると 0.7 秒前後まで延びた。CI の ubuntu では 0.13 秒）） |
