...
- 1 枚あたり約 0.3 秒（1 枚目は約 0.35〜0.5 秒（単独ビルドで実測。別のビルドが並行していると 0.7 秒前後まで延びた。CI の ubuntu では 0.13 秒））。**公開記事の全件を毎回描き直す**ので、公開記事が 100 本なら約 30 秒、約 1,500 本で公開ワークフローの 10 分の制限に近づく（手当ては `../.scratch/pipeline/M12-og-cache.md`）
